...
#include "pipeline/BatchedJoinPipeline.hpp"
#include "pipeline/PipelineFunction.hpp"
//---------------------------------------------------------------------------
namespace engine {
//---------------------------------------------------------------------------
template <size_t... Is>
auto genProbesBatched(engine::span<const DefaultProbeParameter>& probeParams, std::index_sequence<Is...>) {
...
void PipelineFunctions::runPipeline(TargetBase& targetBase, ScanBase& scanBase, engine::span<const DefaultProbeParameter> probeParams, engine::span<const unsigned> keyOffsets, engine::span<const unsigned> attrOffsets) {
    ...
    for (size_t i = 0; i < Attrs::size(); ++i)
        ao[i] = attrOffsets[i];
    auto probes = genProbesBatched(probeParams, std::make_index_sequence<NumJoins>{});

    if constexpr (NumJoins > 0) {
        BatchedJoinPipeline<Target, Scan, decltype(probes), Keys, Attrs> pipeline{target, scan, probes, ko, ao};
        pipeline();
    } else {
